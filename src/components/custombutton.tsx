...
export default function CustomButton({
  ...
  return (
    <button
      type={type}
      className={`group relative mt-4 flex h-9 cursor-pointer items-center justify-center overflow-hidden rounded-md border px-3 font-medium shadow-2xl transition-all before:absolute before:top-0 before:bottom-0 before:left-0 before:z-0 before:h-full before:w-0 before:transition-all before:duration-500 hover:before:left-0 hover:before:w-full ${buttonColorsClass[color][0]} ${className}`}
      onClick={onClick}
    >
      {loading ? (
